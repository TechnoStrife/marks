<template>
    <div class="teachers-summary-chart row" v-if="all_years.length > 0">
        <div class="col s12">
            <ChartCardMulti
                :data="chart_data"
                :options="options"
...
export default {
    ...
    computed: {
        all_years() {
            return distinct(this.data.map(teacher => teacher.year)).reverse()
        },
        selected_year() {
        ...
        chart_data() {
            return this.filtered_teachers.map(
                teacher_marks => ({
                    key: teacher_marks.teacher.id,
                    label: short_name(teacher_marks.teacher.full_name),
                    datasets: [
                        round2(teacher_marks.mark || 0),
                        round2(teacher_marks.terminal_mark || 0),
                        Math.max(round2(teacher_marks.diff || 0), 0),
                        -Math.min(round2(teacher_marks.diff || 0), 0),
                    ],
                })
            )
        },
